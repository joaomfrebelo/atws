...
class InvoiceWsTest extends TestCase
{
    ...
    {
        (new Base(InvoiceWs::class))->testReflection(InvoiceWs::class);
    ...
    public function testXml(): void
    {
        foreach ($this->invoiceDataProvider() as $invoice) {
            ...
            $this->assertSame(
                $data->getDocumentStatus()->getInvoiceStatusDate()->format(Pattern::DATE_T_TIME),
            ...
            foreach ($data->getLines() as $k => $line) {
                ...
                foreach ($line->getOrderReference() ?: [] as $n => $sourceID) {
                    ...
                    $this->assertSame(
                        $sourceID->getOrderDate()->format(Pattern::SQL_DATE),
                        (string)(($xml->xpath(
                            \sprintf(
